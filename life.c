...
int main(int argc, char **argv) {
  char **field, **field2;
  MEVENT event;
  int i, j, a, b, a2, b2, n, k, w=0, con_width, con_height, top_border=1, FPS=5, iter=0;
  float d = 0.2, g = -1.0;
  ...
  while ((k = getopt(argc, argv, "i:d:c:g:wb:s:")) != -1) {
	...
	break;
      case '?':
	if (optopt == 'i' || optopt == 'd' || optopt == 'c' || optopt == 'g' || optopt == 'b' || optopt == 's')
 ...
 input:
  ...
  while ((k = kbhit()) != ' ' && k != 'q') {
	...
	n = 0;
	/* Count neighbors */
	for (a = (w ? i-1 : max(top_border,i-1)); a <= (w ? i+1 : min(con_height-1, i+1)); a++)
	  for (b = (w ? j-1 : max(0,j-1)); b <= (w ? j+1 : min(con_width-1, j+1)); b++) {
	    a2 = (a < top_border) ? con_height-1 : ((a >= con_height) ? top_border : a);
